...
describe("BallotArchiveNativeTest", function () {
  ...
  it('Write and read singular ballot', async() => {
    ...
    assert(_ballot === storedBallot, "Expected address " + _ballot + " but received " + storedBallot);
  });
  it('Write and read multiple ballots', async() => {
    ...
    assert(storedBallots[0] === _ballot1, "Expected address " + _ballot1 + " but received " + storedBallots[0]);
    assert(storedBallots[1] === _ballot2, "Expected address " + _ballot2 + " but received " + storedBallots[1]);
    assert(storedBallots[2] === _ballot3, "Expected address " + _ballot3 + " but received " + storedBallots[2]);
  });
  it('Write and read non-existing ballot', async() => {
    ...
    assert(_expectedBallot === zero_address, "Expected " + zero_address + " but got " + _expectedBallot);
  });
