...
describe("BallotIntegrationTest", function () {
  ...
  it("Deploy and emit singular OpenBallot and read from ballots", async () => {
    ...
    var ballotOpenAddress = await ballotArchiveContract.getBallotByAddress(
  ...
  it("Deploy and emit singular OpenBallot and read from creators", async () => {
    ...
    var ballotOpenAddress = await ballotArchiveContract.getBallotsByCreator(
      voter1.address
    );
    assert(ballotOpenAddress == ballotOpenContract.address);
  });
  it("Deploy and emit multiple OpenBallot and read from ballots", async () => {
    ...
    const ballotOpen1 = await BallotOpen1.connect(voter1).deploy(
      _archiveAddress,
      _title,
      _metainfo,
      _votingDays
    ...
    const ballotOpen2 = await BallotOpen2.connect(voter2).deploy(
      _archiveAddress,
      _title,
      _metainfo,
      _votingDays
    ...
    var ballotOpenAddress1 = await ballotArchiveContract.getBallotByAddress(
      ballotOpenContract1.address
    );
    assert(ballotOpenAddress1 == ballotOpenContract1.address);

    var ballotOpenAddress2 = await ballotArchiveContract.getBallotByAddress(
      ballotOpenContract2.address
    );
  ...
  it("Deploy and emit multiple OpenBallot and read from ballots", async () => {
    ...
    var ballotOpenAddress1 = await ballotArchiveContract.getBallotsByCreator(
      voter1.address
    );
    assert(ballotOpenAddress1 == ballotOpenContract1.address);

    var ballotOpenAddress2 = await ballotArchiveContract.getBallotsByCreator(
      voter2.address
    );
    assert(ballotOpenAddress2 == ballotOpenContract2.address);
  });
